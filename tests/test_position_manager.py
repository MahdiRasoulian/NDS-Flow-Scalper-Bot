...
def test_trailing_updates_sl_only_after_tp1_phase():
    config = {
        "risk_settings": {"TP2_ENABLED": True},
        "flow_settings": {
            "FLOW_TP1_PARTIAL_CLOSE_PCT": 0.5,
            "FLOW_TP1_MOVE_SL_TO_BE": True,
            "FLOW_TRAIL_AFTER_TP1": True,
            "FLOW_TRAIL_DISTANCE_PIPS": 10.0,
        },
        "trading_settings": {"GOLD_SPECIFICATIONS": {"MIN_LOT": 0.01, "LOT_STEP": 0.01, "POINT": 0.01}},
    }
    metadata = {"tp1_price": 2005.0, "tp2_price": 2015.0}
    mt5 = DummyMT5()
    manager = PositionManager(config, mt5, trade_tracker=DummyTradeTracker(metadata))
    ...
    open_positions[0]["current_price"] = 2010.0
    open_positions[0]["sl"] = 2000.0
    manager.manage_positions(open_positions)

    assert any(abs(float(call["new_sl"]) - 2009.0) < 1e-6 for call in mt5.modified if call["new_sl"] is not None)
