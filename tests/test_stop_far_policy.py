...
def test_stop_far_policy_rejects_hard_cap():
    ...
    assert finalized.order_type in {"NONE", "STOP", "MARKET"}


def test_stop_far_policy_caps_entry_on_mean_reversion_without_limit():
    risk_manager = _build_risk_manager()
    cfg = _build_config_payload()
    ...
    assert finalized.order_type in {"STOP", "MARKET"}
    assert finalized.entry_price <= analysis["entry_level"]
